// This is a code to calculate 1-dimensional pair count.
...
struct rec
{
  ...
  *da = *&data;
  ...
  printf("%f\t%f\t%f\n",data[100][0],data[100][1],data[100][2]);
  printf("%f\t%f\t%f\n",data[1000][0],data[1000][1],data[1000][2]);
  printf("%f\t%f\t%f\n",data[10000][0],data[10000][1],data[10000][2]);
  fclose(fp);
  return ndata;
}
...
/*  cross = 0;
  ...
  fileout = "RR";
  sprintf(RR,"radial%s%d_%s_%s.txt",fileout,nproc,select,num_R);
  out1 = fopen(RR,"w");
  fprintf(out1,"%f\n",BIN_MIN);
  fprintf(out1,"%f\n",BIN_MAX);
  fprintf(out1,"%f\n",BIN_WIDTH);
