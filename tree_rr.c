...
*/
  ...
  *&right_hrect.hrect[0][0] = *&right_data[0][0];
  ...
  while (stacksize != 0){
	...
	for (j = 0; j < ndata; j++){
      		for (i = 0; i < 3; i++){
	  		leaf_data.leafpoints[j][i]=(data_tmp[j][i]);
		}
	  	leaf_data.leafpoints[j][3]=sqrt(leaf_data.leafpoints[j][0]*leaf_data.leafpoints[j][0]+leaf_data.leafpoints[j][1]*leaf_data.leafpoints[j][1]+leaf_data.leafpoints[j][2]*leaf_data.leafpoints[j][2]);
	}
	if(ndata < leafsize){
...
long **dualTreeParallel(tree_st *tree1, tree_st *tree2,  int len_tree1, int len_tree2, hrect_st *hrectree1, hrect_st *hrectree2, leaf_st *leaf_points1, leaf_st *leaf_points2, int *nodelist1, int nodesize1, int *nodelist2, int nodesize2, int cross, float BIN_MIN, float BIN_MAX, float BIN_WIDTH, int nproc)
{
  ...
  bin = calloc(numbins*numbins,sizeof(long));
  for(i=0;i<numbins;i++) bins[i] = &bin[i*numbins];
  if(cross == 1){
  ...
  #pragma omp parallel private (n,temp,temps,counts,count, node1, node2, i,j,k,l) shared(bins,tree1,tree2,hrectree1,hrectree2,leaf_points1,leaf_points2,len_tree1,len_tree2)
  {
    ...
    temps = calloc(numbins,sizeof(long*));
    temp = calloc(numbins*numbins,sizeof(long));
    count = calloc(numbins*numbins,sizeof(long));
    for(k=0;k<numbins;k++){
...
long **dualTreeCountAuto(float BIN_MIN, float BIN_MAX, float BIN_WIDTH, tree_st *tree1,  int len_tree1,  hrect_st *hrectree1, \
  ...
  int leaf_ptr1, leaf_ptr2,rpara_idx,rperp_idx,maxbins2;
  float dmin, dmax, dist,rpara,rperp,mu;
  ...
  stack[0] = node_idx2;
  stack[1] = node_idx1;
  maxbins2 = ((int)(1.42*numbins)+1)*BIN_WIDTH; 
  ...
  while(stacksize >= 0){
    ...
    if (idx2 <= idx1){
      ...
      dmin = pow((pow(float_max(0.,float_max(hrect1.hrect[0][0]-hrect2.hrect[1][0],hrect2.hrect[0][0]-hrect1.hrect[1][0])),2.) + \
		  pow(float_max(0.,float_max(hrect1.hrect[0][1]-hrect2.hrect[1][1],hrect2.hrect[0][1]-hrect1.hrect[1][1])),2.) + \
		   pow(float_max(0.,float_max(hrect1.hrect[0][2]-hrect2.hrect[1][2],hrect2.hrect[0][2]-hrect1.hrect[1][2])),2.)),0.5);
      if (dmin <= BIN_MAX){
	dmax = pow((pow(float_max(hrect1.hrect[1][0]-hrect2.hrect[0][0],hrect2.hrect[1][0]-hrect1.hrect[0][0]),2.) + \
		     pow(float_max(hrect1.hrect[1][1]-hrect2.hrect[0][1],hrect2.hrect[1][1]-hrect1.hrect[0][1]),2.) + \
		     pow(float_max(hrect1.hrect[1][2]-hrect2.hrect[0][2],hrect2.hrect[1][2]-hrect1.hrect[0][2]),2.)),0.5);
	if(dmax >= BIN_MIN){
	  if(idx1 == idx2){
...
//	    else if(leaf_ptr1 != -1){
		...
		for(j=i+1; j <= numpoints1-1; j++){
		  dist = pow((pow((leaf_points1[leaf_ptr1].leafpoints[i][0]-leaf_points1[leaf_ptr1].leafpoints[j][0]),2.) + \
			       pow((leaf_points1[leaf_ptr1].leafpoints[i][1]-leaf_points1[leaf_ptr1].leafpoints[j][1]),2.) + \
			       pow((leaf_points1[leaf_ptr1].leafpoints[i][2]-leaf_points1[leaf_ptr1].leafpoints[j][2]),2.)),0.5);
		  if(dist > BIN_MIN){
		   // r_index = (int)((dist-BIN_MIN)/BIN_WIDTH);
		    if(dist < maxbins2){
		      mu = fabs(leaf_points1[leaf_ptr1].leafpoints[i][3]-leaf_points1[leaf_ptr1].leafpoints[j][3])*sqrt(0.5D+0.5D*(leaf_points1[leaf_ptr1].leafpoints[i][0]*leaf_points1[leaf_ptr1].leafpoints[j][0]+leaf_points1[leaf_ptr1].leafpoints[i][1]*leaf_points1[leaf_ptr1].leafpoints[j][1]+leaf_points1[leaf_ptr1].leafpoints[i][2]*leaf_points1[leaf_ptr1].leafpoints[j][2])/(leaf_points1[leaf_ptr1].leafpoints[i][3]*leaf_points1[leaf_ptr1].leafpoints[j][3]))/dist;
			if(mu<1){
			rpara = dist*mu;
			rperp = dist*sqrt(1.-mu*mu);
			rpara_idx = (int)(rpara/BIN_WIDTH);
			rperp_idx = (int)(rperp/BIN_WIDTH);
//		      mu_idx = (int)(20.*mu);
//			mu_idx+=10;
//		      if(mu_idx == 20) mu_idx -= 1;
		      //printf("mu_idx : %d\n",mu_idx);
			if((rperp_idx < numbins) &&(rpara_idx < numbins)){
			      bins[rpara_idx][rperp_idx] += 1;
			}
			}
			else{
				printf("mu is larger than 1!\n");
			}
		    }
		  }
		}
...
//idx2 < idx1
		...
		for(j=0; j <= numpoints2-1; j++){
		  dist = pow((pow((leaf_points1[leaf_ptr1].leafpoints[i][0]-leaf_points1[leaf_ptr2].leafpoints[j][0]),2.) + \
			       pow((leaf_points1[leaf_ptr1].leafpoints[i][1]-leaf_points1[leaf_ptr2].leafpoints[j][1]),2.) + \
			       pow((leaf_points1[leaf_ptr1].leafpoints[i][2]-leaf_points1[leaf_ptr2].leafpoints[j][2]),2.)),0.5);
		 
		  if(dist > BIN_MIN){
//		    r_index = (int)((dist-BIN_MIN)/BIN_WIDTH);
//		    if(r_index < numbins){
		    if(dist < maxbins2){
		      mu = fabs(leaf_points1[leaf_ptr1].leafpoints[i][3]-leaf_points1[leaf_ptr2].leafpoints[j][3])*sqrt(0.5+0.5*(leaf_points1[leaf_ptr1].leafpoints[i][0]*leaf_points1[leaf_ptr2].leafpoints[j][0]+leaf_points1[leaf_ptr1].leafpoints[i][1]*leaf_points1[leaf_ptr2].leafpoints[j][1]+leaf_points1[leaf_ptr1].leafpoints[i][2]*leaf_points1[leaf_ptr2].leafpoints[j][2])/(leaf_points1[leaf_ptr1].leafpoints[i][3]*leaf_points1[leaf_ptr2].leafpoints[j][3]))/dist;
//		      mu_idx = (int) (20.*mu);
			if(mu<1){
			rpara = dist*mu;
			rperp = dist*sqrt(1.-mu*mu);
...
long **dualTreeCountCross(float BIN_MIN, float BIN_MAX, float BIN_WIDTH, tree_st *tree1, tree_st *tree2, int len_tree1, int len_tree2, hrect_st *hrectree1, \
  ...
  int stacksize = 1;
  int idx1, idx2, numpoints1, numpoints2, left1, right1, left2, right2, i, j, r_index;
  int leaf_ptr1, leaf_ptr2,mu_idx,rpara_idx,rperp_idx,maxbins2;
  float dmin, dmax, dist, z, mu,rpara,rperp;
  hrect_st hrect1, hrect2;
  long **bins,*bin;
...
//      printf("test %d %f %f %f %f\n",i,leaf_points1[leaf_ptr1].leafpoints[i][0],leaf_points1[leaf_ptr1].leafpoints[i][1],leaf_points1[leaf_ptr1].leafpoints[i][2],leaf_points1[leaf_ptr1].leafpoints[i][3]);
	      dist = pow((pow((leaf_points1[leaf_ptr1].leafpoints[i][0]-leaf_points2[leaf_ptr2].leafpoints[j][0]),2.) + \
			   pow((leaf_points1[leaf_ptr1].leafpoints[i][1]-leaf_points2[leaf_ptr2].leafpoints[j][1]),2.) + \
			   pow((leaf_points1[leaf_ptr1].leafpoints[i][2]-leaf_points2[leaf_ptr2].leafpoints[j][2]),2.)),0.5);
	      if(dist >= BIN_MIN){
//		r_index = (int)((dist-BIN_MIN)/BIN_WIDTH);
		
//		if(r_index < numbins2){
		if(dist < maxbins2){
		      mu = fabs(leaf_points1[leaf_ptr1].leafpoints[i][3]-leaf_points2[leaf_ptr2].leafpoints[j][3])*sqrt(0.5D+0.5D*(leaf_points1[leaf_ptr1].leafpoints[i][0]*leaf_points2[leaf_ptr2].leafpoints[j][0]+leaf_points1[leaf_ptr1].leafpoints[i][1]*leaf_points2[leaf_ptr2].leafpoints[j][1]+leaf_points1[leaf_ptr1].leafpoints[i][2]*leaf_points2[leaf_ptr2].leafpoints[j][2])/(leaf_points1[leaf_ptr1].leafpoints[i][3]*leaf_points2[leaf_ptr2].leafpoints[j][3]))/dist;
			if(mu < 1.0){
			rpara = dist*mu;
			rperp = dist*sqrt(1.-mu*mu);
			rpara_idx = (int)(rpara/BIN_WIDTH);
			rperp_idx = (int)(rperp/BIN_WIDTH);
			if((rperp_idx < numbins) &&(rpara_idx < numbins)){
			//	printf("mu%f para %d perp %d\n",mu,rpara_idx,rperp_idx);
			      bins[rpara_idx][rperp_idx] += 1;
			}
			}
			else{
...
int main(){
  ...
  bin = calloc(numbins*numbins,sizeof(long));
  for(i=0;i<numbins;i++) bins1[i] = &bin[i*numbins];
  tree_st *tree1, *tree2;
